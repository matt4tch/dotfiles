...
# Resolve a path to its canonical absolute form. Works on both GNU and BSD
# readlink (both support -f in the versions we target).
_canon() {
...
# _convert_symlink_dir_to_real <path>
#   If <path> is a symlink, back it up (via backup_name) and make room for a
#   real directory. No-op if <path> is already a real dir or missing.
...
link_codex() {
  ...
  _convert_symlink_dir_to_real "$home_codex"
  ...
  if [ -d "$repo_codex/skills" ]; then
    ...
    _convert_symlink_dir_to_real "$home_skills"
...
link_gh() {
  ...
  _convert_symlink_dir_to_real "$home_gh"
